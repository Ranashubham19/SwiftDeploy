...
app.get('/ops/status', requireAuth, (req, res) => {
  ...
  const hasGemini = Boolean((process.env.API_KEY || '').trim());
  const apiConfigured = provider === 'openrouter'
    ? hasOpenRouter || hasOpenAI || hasAnthropic || hasGemini
    : provider === 'openai'
      ? hasOpenAI || hasOpenRouter || hasAnthropic || hasGemini
      : provider === 'anthropic'
        ? hasAnthropic || hasOpenRouter || hasOpenAI || hasGemini
        : hasGemini || hasOpenRouter || hasOpenAI || hasAnthropic;
  const stripeConfigured = (process.env.STRIPE_SECRET_KEY || '').trim().startsWith('sk_');
  const mode = isProduction ? 'production' : 'development';
  const usingHttpsBaseUrl = /^https:\/\//i.test(BASE_URL);

  return res.json({
    success: true,
    mode,
    baseUrl: BASE_URL,
    webhookReady: isProduction ? usingHttpsBaseUrl : true,
    aiConfigured: apiConfigured,
    aiProvider: provider,
    stripeConfigured,
    deployedBots: botTokens.size + discordBots.size,
    timestamp: new Date().toISOString()
  });
});

app.get('/ops/live-data', requireAuth, (req, res) => {
  const reqUser = req.user as Express.User | undefined;
  const userEmail = (reqUser?.email || '').trim().toLowerCase();
  if (!userEmail) {
  ...
  const telegramIds = Array.from(botTokens.keys()).filter((id) => {
    const owner = telegramBotOwners.get(id);
    return !owner || owner === userEmail;
  });
  const discordIds = Array.from(discordBots.entries())
    .filter(([, cfg]) => cfg.createdBy === userEmail)
    .map(([id]) => id);
  const allBotIds = Array.from(new Set([...telegramIds, ...discordIds]));

  const botItems = allBotIds.map((botId) => {
    const discordCfg = discordBots.get(botId);
    const isDiscord = Boolean(discordCfg);
    const platform: BotPlatform = isDiscord ? 'DISCORD' : 'TELEGRAM';
    const telemetry = ensureBotTelemetry(botId, platform, userEmail);
    const avgLatencyMs = telemetry.latencySamples > 0
      ? Math.round(telemetry.totalLatencyMs / telemetry.latencySamples)
      : 0;
    const gatewayConnected = isDiscord ? Boolean(discordGatewayClients.get(botId)?.isReady()) : undefined;
    const recentActive = telemetry.lastActiveAt ? (Date.now() - new Date(telemetry.lastActiveAt).getTime()) < 10 * 60 * 1000 : false;
    const status = isDiscord
      ? (gatewayConnected ? 'ONLINE' : 'OFFLINE')
      : (recentActive ? 'ONLINE' : 'IDLE');

    const webhookOrInteractionUrl = isDiscord
      ? `${BASE_URL}/discord/interactions/${botId}`
      : `${BASE_URL}/webhook/${botId}`;
    const managementUrl = isDiscord
      ? `https://discord.com/developers/applications/${discordCfg?.applicationId || ''}`
      : 'https://t.me';
    const monitoringUrl = isDiscord
      ? `${BASE_URL}/discord/bot-status/${botId}`
      : `${BASE_URL}/get-webhook-info`;

    return {
      botId,
      platform,
      status,
      gatewayConnected: isDiscord ? gatewayConnected : undefined,
      ...
      lastErrorMessage: telemetry.lastErrorMessage,
      webhookOrInteractionUrl,
      managementUrl,
      monitoringUrl
    };
  });

  const selectedBot = (requestedBotId
    ? botItems.find((item) => item.botId === requestedBotId)
    : botItems[0]) || null;

  const totals = botItems.reduce((acc, item) => {
    acc.messageCount += item.messageCount;
    acc.responseCount += item.responseCount;
    acc.errorCount += item.errorCount;
    acc.tokenUsage += item.tokenUsage;
    acc.latencyWeighted += item.avgLatencyMs * Math.max(item.responseCount, 1);
    acc.responseWeight += Math.max(item.responseCount, 1);
    if (item.lastActiveAt && (!acc.lastEventAt || new Date(item.lastActiveAt).getTime() > new Date(acc.lastEventAt).getTime())) {
      acc.lastEventAt = item.lastActiveAt;
    }
    return acc;
  }, {
    messageCount: 0,
  ...
  return res.json({
    ...
    timestamp: new Date().toISOString(),
    overview: {
      deployedBots: botItems.length,
      onlineBots: botItems.filter((item) => item.status === 'ONLINE').length,
      messageCount: totals.messageCount,
      responseCount: totals.responseCount,
      errorCount: totals.errorCount,
      tokenUsage: totals.tokenUsage,
      avgLatencyMs: totals.responseWeight > 0 ? Math.round(totals.latencyWeighted / totals.responseWeight) : 0,
